	...
	subscriptions = Comms:BulkSubscribe(addon.PREFIXES.MAIN, {
		...
		lootTable = function (_, sender)
			if addon:UnitIsUnit(sender, "player") then
				-- WotLK: extended from 11s to 60s so it matches the loot
				-- frame duration. Players who haven't clicked yet stay
				-- "waiting" instead of being mislabeled "Offline".
				self:ScheduleTimer("Timer", 60 + 0.5*#self.lootTable, "LootSend")
			end
		end,
