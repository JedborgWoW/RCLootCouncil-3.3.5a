...
function RCLootCouncil:AddSpecIconToText(specID, text, size)
	...
	-- ASCENSION FIX: retail spec icons don't exist here; GetSpecializationInfoByID
	-- returns nil, leaving specIcon nil and producing a broken tiny texture.
	-- Fall back to just the text (class coloring is already applied).
...
function RCLootCouncil:LogItemGUID(item)
	-- WotLK has no item GUIDs, so this is always nil and just spams the log.
	-- Only log when an actual GUID exists (i.e. never on 3.3.5a).
	local guid = self.ItemStorage:GetItemGUID(item)
	if guid then self.Log:D("Item GUID: " .. tostring(guid)) end
end
