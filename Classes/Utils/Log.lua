...
function private:Print(msg, ...)
	-- luacov: disable
	-- WotLK fix: addon:Print is RawHook'd in core.lua to ALSO call Log:f("<PRINT>")
	-- on every invocation. If debug is on, private:Log calls this Print, which
	-- calls addon:Print, which calls Log:f -> private:Log -> private:Print ...
	-- an infinite loop that grows a string until "memory allocation error:
	-- block too big". Break the loop by using the UNHOOKED original print.
	local prefix = "|cffcb6700debug:|r " .. tostring(msg)
	if addon.hooks and addon.hooks[addon] and addon.hooks[addon].Print then
		-- Call the original (pre-hook) Print directly — does NOT re-trigger Log:f.
		if select("#", ...) > 0 then
			addon.hooks[addon].Print(addon, getglobal(addon.db.profile.chatFrameName) or DEFAULT_CHAT_FRAME, prefix .. "|cffff6767", ...)
		else
			addon.hooks[addon].Print(addon, getglobal(addon.db.profile.chatFrameName) or DEFAULT_CHAT_FRAME, prefix .. "|r")
		end
		return
	end
	-- Fallback before the hook exists: plain chat print.
	if select("#", ...) > 0 then
		addon:Print(prefix .. "|cffff6767", ...)
	else
		addon:Print(prefix .. "|r")
	end
	-- luacov: enable
end
