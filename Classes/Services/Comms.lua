...
function private.ReceiveComm(prefix, encodedMsg, distri, sender)
	...
	-- Unpack message
	-- ASCENSION FIX: use EncodeForPrint/DecodeForPrint (printable-ASCII
	-- range) instead of EncodeForWoWAddonChannel. Ascension's addon
	-- message channel appears to drop messages containing the wider byte
	-- range the WoWAddonChannel encoder produces; the printable encoding
	-- (which the old working Ascension port used) survives delivery.
	local decoded = ld:DecodeForPrint(encodedMsg)
...
function private:GetGroupChannel()
	-- INSTANCE_CHAT doesn't exist on 3.3.5a; use only valid WotLK channels.
	if IsInRaid() then
		return "RAID"
