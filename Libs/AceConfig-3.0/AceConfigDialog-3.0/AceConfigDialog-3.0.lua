...
local function safecall(func, ...)
	if func then
		-- WotLK / Lua 5.1: xpcall does NOT forward extra arguments after the
		-- error handler (that's a 5.2+ feature). Passing (func, errorhandler,
		-- ...) silently drops every argument, so option setters receive a nil
		-- `info` table and crash with "attempt to get length of local 'info'".
		-- Bind the args into a closure so they survive on 5.1.
		local args = { ... }
		local n = select("#", ...)
