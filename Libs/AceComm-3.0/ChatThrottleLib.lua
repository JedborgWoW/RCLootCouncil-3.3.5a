...
local function PerformSend(sendFunction, ...)
	bMyTraffic = true
	-- Lua 5.1 (WotLK): xpcall doesn't forward args; bind in a closure.
	local args = { ... }
	local n = select("#", ...)
	local sendResult = MapToSendResult(xpcall(function() return sendFunction(unpack(args, 1, n)) end, CallErrorHandler))
	bMyTraffic = false
	return sendResult
end
...
function ChatThrottleLib.OnUpdate(this,delay)
	...
	if self.OnUpdateDelay < 0.1 then
		return
	end
	self.OnUpdateDelay = 0

	-- ASCENSION FIX: the original bandwidth-gauge math (UpdateAvail using
	-- GetTime/GetFramerate) stalls on this core, leaving messages queued
	-- for minutes. Replace it with a simple paced drain: send up to a few
	-- queued messages every ~0.1s (≈ a handful per second), which clears
	-- bursts quickly while staying under the server's per-prefix limit.
	local SEND_PER_TICK = 3
	local sentThisTick = 0
	local anyLeft = false

	for _, Prio in pairs(self.Prio) do
		-- Fold any blocked pipes back into the active ring so throttled
		-- messages get retried.
		if Prio.Blocked and Prio.Blocked.pos then
			Ring_Link(Prio.Ring, Prio.Blocked)
		end

		while Prio.Ring.pos and sentThisTick < SEND_PER_TICK do
			local pipe = Prio.Ring.pos
			local msg = pipe[1]
			local ok = pcall(msg.f, unpack(msg, 1, msg.n))
			-- Treat as sent regardless (WotLK SendAddonMessage returns
			-- nothing); dequeue and move on.
			table_remove(pipe, 1)
			DelMsg(msg)
			...
			if msg.callbackFn then
				securecallfunction(msg.callbackFn, msg.callbackArg, ok and true or false,
					(Enum and Enum.SendAddonMessageResult and Enum.SendAddonMessageResult.Success) or 0)
			end
		end

		if Prio.Ring.pos then anyLeft = true end
	end

	-- Stop the OnUpdate driver once everything is drained.
	if not anyLeft then
...
function ChatThrottleLib:Enqueue(prioname, pipename, msg)
	...
	self.bQueueing = true
	self.Frame:Show()  -- ASCENSION FIX: always (re)start the drain loop
end
...
local function SendAddonMessageInternal(self, sendFunction, prio, prefix, text, chattype, target, queueName, callbackFn, callbackArg)
	...
	-- ASCENSION FIX: always queue and let the paced OnUpdate drain send.
	-- The original immediate-send path relied on UpdateAvail()'s bandwidth
	-- math, which stalls on this core. Queueing everything routes all
	-- sends through our reliable fixed-rate drain (see OnUpdate).
	local msg = NewMsg()
	msg.f = sendFunction
	msg[1] = prefix
	msg[2] = text
...
function ChatThrottleLib:SendAddonMessage(prio, prefix, text, chattype, target, queueName, callbackFn, callbackArg)
	...
	-- ASCENSION FIX: use the global SendAddonMessage directly instead of
	-- C_ChatInfo.SendAddonMessage. On Ascension the C_ChatInfo path does
	-- not actually deliver messages to other clients (other addons like
	-- DBM/LibGroupTalents work because they use the global), so route
	-- through the global, which is the proven-working path on this core.
	local sendFunction = _G.SendAddonMessage or _G.C_ChatInfo.SendAddonMessage
